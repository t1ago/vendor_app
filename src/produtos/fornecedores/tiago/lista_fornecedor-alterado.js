...
buscar_dados_filtrado = async function (name) {
    requisicao = await fetch(`${API_HOST}/fornecedores/tiago?name=${name}`, {
        method: 'GET'
    })
...
montar_tabela = function (lista_fornecedores) {
    ...
    adicionar_botao = function (coluna, url_imagem, funcao) {
    ...
    lista_fornecedores.forEach((item) => {
        ...
        coluna = document.createElement('td')
        botao_editar = adicionar_botao(linha, '../../../../imagens/editar.png', function () {
            botao_editar_click(item.id)
        })

        botao_remover = adicionar_botao(linha, '../../../../imagens/remover.png', function () {
            console.log(item.id)
        })
...
filtrar_dados = function (event) {
    ...
    timeout = setTimeout(async function () {
        exibir_situacao_operacao('BUSCANDO')
        lista_fornecedores = await buscar_dados_filtrado(value)
